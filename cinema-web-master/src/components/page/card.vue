...
-->
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item> <i class="el-icon-lx-cascades"></i> {{ name }}的会员卡列表 </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button type="primary" icon="el-icon-bank-card" class="handle-del mr10" @click="memberVisible = true"
                    >会员选择
                </el-button>
                <el-button type="primary" icon="el-icon-bank-card" class="handle-del mr10" @click="handleRegister">会员卡注册 </el-button>
                ...
                <span v-if="isBirthday" class="birthday">
                    <el-tag type="danger" effect="dark" style="margin-left: 10px"> 今天是用户{{ this.name }}的生日 </el-tag>
                </span>
            ...
            >
                ...
                <el-table-column prop="lose" label="状态" align="center">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.lose === 0 ? 'success' : 'danger'">{{ scope.row.lose === 0 ? '正常' : '挂失' }} </el-tag>
                ...
                <el-table-column label="操作" width="500px" align="center">
                    <template slot-scope="scope">
                        <el-button type="primary" @click="handleRecharge(scope.$index, scope.row)" :disabled="scope.row.lose !== 0"
                            >充值
                        </el-button>
                        <el-button
                            type="primary"
                        ...
                        <el-button type="warning" v-if="scope.row.lose === 0" @click="handleLoss(scope.$index, scope.row)">挂失 </el-button>
                        <!--点击时传递当前行索引和数据给函数，并在函数中赋值给form等数据，
                        并将编辑框显示-->
                        <el-button type="danger" v-if="scope.row.lose === 1" @click="handleCancel(scope.$index, scope.row)"
                            >解挂
                        </el-button>
                        <el-button type="danger" v-if="scope.row.lose === 1" @click="handleReissue(scope.$index, scope.row)"
                            >补卡
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="交易记录查询" :visible.sync="recordVisible" width="80%">
            <el-table :data="recordData" border>
                ...
                <el-table-column prop="spendType" label="交易类型" align="center">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.spendType === 0 ? 'success' : 'danger'"
        ...
        <el-dialog title="消费" :visible.sync="consumeVisible" width="35%">
            <el-form :model="form" :label-position="labelPosition" label-width="80px" :rules="rules">
                ...
                <el-form-item label="电影票">
                    <el-select v-model="form.movieId" placeholder="请选择电影" @change="pickMoviePriceAndIntegral" label-width="80px">
                        ...
                        <el-option v-for="item in movieData" :key="item.movieId" :label="item.name" :value="item.movieId">
...
export default {
    ...
    methods: {
        ...
        getPlanTypeData(val, fun) {
            ...
            getCardIdByFuzzyQuery(query).then((res) => {
                //发送请求
