        ...
        getMemberData() {
            memberData(this.query).then((res) => {
                ...
                this.memberData.map((m) => {
                    // 拿到该用户的生日
                    let myDate = new Date(m.birthday.replace(/-/g, '/')); // "2010/08/01";
                    // 获取该用户所在的年份
                    let oldDataYear = myDate.getFullYear();
                    // 获取当前的日期
                    let nowDate = new Date();
                    let newDateMonth = nowDate.getMonth() + 1;
                    let newDateDate = nowDate.getDate();
                    // 利用字符串拼接生成新的日期，也就是把当前日期的年份换成该用户所在的年份
                    let newDateStr = oldDataYear + '-' + newDateMonth + '-' + newDateDate;
                    let newDate = new Date(newDateStr);
                    // 求出距离生日的天数，注意要用绝对值
                    m.instance = parseInt(Math.abs(newDate - myDate) / (1000 * 60 * 60 * 24)) + 1; //核心：时间戳相减，然后除以天数
                });
